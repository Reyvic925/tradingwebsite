...
export default function Dashboard() {
  ...
  const investmentSummary = investments.reduce((summary, investment) => {
    ...
    const value = Number(investment.current_value ?? principal + Number(investment.earned || 0));
    summary.totalPnl += value - principal;
    if (investment.status === 'completed') summary.completed += 1;
    else summary.active += 1;
    return summary;
  }, { totalPnl: 0, active: 0, completed: 0 });
    ...
    <AppShell>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-md border border-white/5">
          ...
          {(investmentSummary.active > 0 || investmentSummary.completed > 0) && (
            <div className="border-b border-white/5 px-5 py-3 text-xs">
              <div className="mb-2 rounded bg-amber-500/10 p-2 text-amber-200">
                <strong>{investmentSummary.active}</strong> active {investmentSummary.active === 1 ? 'plan' : 'plans'} • <strong>{investmentSummary.completed}</strong> completed
              </div>
